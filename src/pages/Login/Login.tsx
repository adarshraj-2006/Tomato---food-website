...
const Login = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 md:p-8 font-sans">
      <div className="bg-white w-full max-w-[1000px] h-full min-h-[650px] rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row border border-slate-100">
        ...
        {/* Right Side: Form */}
        <div className="w-full md:w-[60%] p-8 md:p-12 flex flex-col h-full bg-white relative">

          <div className="mb-8">
            <div className="flex bg-slate-100 p-1.5 rounded-2xl w-fit mb-6">
              <button
                onClick={() => setAuthMode("login")}
                className={`px-8 py-2.5 rounded-xl font-bold text-sm transition-all ${authMode === "login" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
                Login
              </button>
              <button
                onClick={() => setAuthMode("signup")}
                className={`px-8 py-2.5 rounded-xl font-bold text-sm transition-all ${authMode === "signup" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
              >
                Sign Up
              </button>
            </div>
            ...
            {authMode === "login" ? (
              <div className="flex gap-3">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 p-4 rounded-2xl border-2 transition-all font-bold ${role === "customer" ? "border-red-500 bg-red-50 text-red-600" : "border-slate-100 hover:border-slate-200 text-slate-600"}`}
                >
                  <User size={20} /> Customer
                </button>
                <button
                  onClick={() => setRole("seller")}
                  className={`flex-1 flex items-center justify-center gap-2 p-4 rounded-2xl border-2 transition-all font-bold ${role === "seller" ? "border-red-500 bg-red-50 text-red-600" : "border-slate-100 hover:border-slate-200 text-slate-600"}`}
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  onClick={() => setRole("customer")}
                  className={`relative p-5 rounded-2xl border-2 transition-all text-left ${role === "customer" ? "border-red-500 bg-red-50" : "border-slate-100 hover:border-slate-200"}`}
                >
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${role === "customer" ? "bg-red-500 text-white" : "bg-slate-100 text-slate-500"}`}>
                    <User size={24} />
                  </div>
                  <h3 className={`font-black ${role === "customer" ? "text-red-600" : "text-slate-900"}`}>Personal</h3>
                  <p className="text-xs text-slate-500 mt-1">To order food & explore</p>
                  {role === "customer" && <CheckCircle2 className="absolute top-4 right-4 text-red-500" size={20} />}
                </button>
                <button
                  onClick={() => setRole("seller")}
                  className={`relative p-5 rounded-2xl border-2 transition-all text-left ${role === "seller" ? "border-red-500 bg-red-50" : "border-slate-100 hover:border-slate-200"}`}
                >
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${role === "seller" ? "bg-red-500 text-white" : "bg-slate-100 text-slate-500"}`}>
                    <Store size={24} />
                  </div>
                  <h3 className={`font-black ${role === "seller" ? "text-red-600" : "text-slate-900"}`}>Business</h3>
                  <p className="text-xs text-slate-500 mt-1">For restaurant partners</p>
            ...
            {authMode === "signup" && role === "customer" && (
              <div className="flex items-center gap-4 mb-4">
                <button
                  ...
                  className={`text-sm font-bold pb-1 transition-all ${method === "email" ? "text-red-500 border-b-2 border-red-500" : "text-slate-400 hover:text-slate-600"}`}
                >
                  With Email
                </button>
                <button
                  ...
                  className={`text-sm font-bold pb-1 transition-all ${method === "phone" ? "text-red-500 border-b-2 border-red-500" : "text-slate-400 hover:text-slate-600"}`}
                >
                  With Phone
                </button>
              </div>
                ...
                role === "customer" ? (
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-red-500/10 focus:border-red-500 focus:bg-white transition-all"
                    />
                  </div>
                ) : (
                  <div className="relative group">
          ...
          <div className="mt-auto pt-8">
            <p className="text-center text-slate-400 text-sm font-medium">
              ...
              <span className="text-slate-900 hover:text-red-500 cursor-pointer transition-colors px-1">Terms</span> &
              <span className="text-slate-900 hover:text-red-500 cursor-pointer transition-colors px-1">Privacy</span>
            </p>
          </div>
