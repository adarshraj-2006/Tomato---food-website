...
const Cart = () => {
  const { items: cartItems, food_list, addToCart, removeFromCart, updateQuantity, getTotalCartAmount } = useContext(CartContext);
  ...
  // Filter items actually in cart
  const activeItems = food_list.filter(item => cartItems[item.id] > 0);
    ...
    return (
      <MainLayout>
        <div className="min-h-[60vh] flex flex-col items-center justify-center p-8 text-center animate-in fade-in zoom-in duration-500">
          <div className="w-24 h-24 bg-red-50 dark:bg-red-900/20 rounded-full flex items-center justify-center text-red-500 mb-6">
            <ShoppingBag size={48} />
          </div>
          <h2 className="text-3xl font-black text-neutral-900 dark:text-white mb-2">Your cart is empty</h2>
          <p className="text-neutral-500 dark:text-neutral-400 mb-8 max-w-sm">Looks like you haven't added anything to your cart yet. Let's find something delicious!</p>
          <button
            onClick={() => navigate('/')}
            className="px-8 py-4 bg-red-500 text-white rounded-2xl font-black shadow-xl shadow-red-500/20 hover:bg-red-600 transition-all active:scale-95 flex items-center gap-2"
          >
